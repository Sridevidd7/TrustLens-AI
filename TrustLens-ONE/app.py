...
def init_db():
    with get_db() as c:
        ...
        ''')
        c.execute(
            "INSERT OR IGNORE INTO settings(key,value) VALUES ('autonomy','Human confirms high-impact actions')"
        )
...
@app.get('/api/fusion')
def fusion():
    """Expose the unified ensemble that powers every TrustLens decision."""
    models = [
        {'name': 'Sentinel', 'role': 'Threat detection', 'score': 96, 'status': 'online',
         'signal': 'Correlating endpoint, identity, and network telemetry'},
        {'name': 'PolicyGuard', 'role': 'Policy validation', 'score': 100, 'status': 'online',
         'signal': 'Testing actions against 42 active controls'},
        {'name': 'RiskGraph', 'role': 'Blast-radius analysis', 'score': 91, 'status': 'online',
         'signal': 'Mapping dependencies across 1,842 resources'},
        {'name': 'ImpactSim', 'role': 'Outcome simulation', 'score': 88, 'status': 'online',
         'signal': 'Comparing approval, rejection, and staged rollout'},
        {'name': 'ExplainAI', 'role': 'Evidence and transparency', 'score': 94, 'status': 'online',
         'signal': 'Grounding claims in traceable data sources'},
    ]
    return jsonify(
        name='TrustLens ONE Fusion Engine',
        version='1.0.0',
        consensus=94,
        trust_score=96,
        models=models,
...
def run_fusion_analysis():
    with get_db() as c:
        count = c.execute('SELECT COUNT(*) FROM recommendations').fetchone()[0]
        c.execute(
            'INSERT INTO audit(time,actor,action,target,result,type) VALUES (?,?,?,?,?,?)',
            (now_str(), 'TrustLens ONE', 'Ran unified model analysis', 'Tenant environment',
             f'{count} recommendations ranked · 5-model consensus', 'ai')
        )
    return jsonify(
        ok=True,
        recommendations=count,
        consensus=94,
        message='Five specialist models reached consensus. Recommendations were re-ranked by risk and impact.',
    )
...
def get_settings():
    with get_db() as c:
        row = c.execute("SELECT value FROM settings WHERE key='autonomy'").fetchone()
    return jsonify(autonomy=row['value'] if row else 'Human confirms high-impact actions')


@app.post('/api/settings')
def update_settings():
    body = request.get_json(silent=True) or {}
    mode = body.get('autonomy', 'Human confirms high-impact actions')
    allowed = (
        'Always ask a human',
    ...
    if mode not in allowed:
        return jsonify(error='Invalid autonomy mode'), 400
    with get_db() as c:
        c.execute("INSERT OR REPLACE INTO settings(key,value) VALUES ('autonomy',?)", (mode,))
        c.execute(
            'INSERT INTO audit(time,actor,action,target,result,type) VALUES (?,?,?,?,?,?)',
            (now_str(), 'Alex Smith', 'Updated autonomy policy', 'Fusion Engine', mode, 'edit')
        )
    return jsonify(ok=True, autonomy=mode)
...
def incidents():
    return jsonify(incidents=[
        {
            'id': 1,
            'title': 'Quarantine recommendation safely intercepted',
            'severity': 'High',
            'what': 'Sentinel proposed isolating a finance endpoint after unusual outbound traffic.',
            'why': 'RiskGraph found a critical payroll dependency and ExplainAI flagged stale threat intelligence.',
            'safeguard': 'The fusion veto paused automation and routed the action to a human.',
            'decision': 'Analyst approved a narrower IP block and requested fresh telemetry.',
            'lesson': 'Require two fresh threat signals before full network quarantine.',
        },
        {
            'id': 2,
            'title': 'Patch rollout automatically narrowed',
            'severity': 'Medium',
            'what': 'A broad critical patch deployment included five offline warehouse devices.',
            'why': 'ImpactSim predicted operational disruption from an unverified device state.',
            'safeguard': 'The engine converted the action into a 10% pilot rollout.',
            'decision': 'Pilot completed successfully; remaining devices await their maintenance window.',
            'lesson': 'Use recent check-in status as a rollout eligibility gate.',
        },
    ])
